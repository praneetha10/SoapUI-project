...
import Reusable.ReuseableMethods;
...
public class Runner {
	...
	public static void main(String[] args) throws Exception
	{
		...
		File f=new File("src\\test\\resources\\datadriven.xlsx");
		...
		for(int i=1;i<nour;i++)
		{
			...
			//Create object to reusable methdos class
			ReuseableMethods obj;
			if(env.equalsIgnoreCase("local"))
			{
				obj=new ReuseableMethods(bn);
			}
			else
			{
				obj=new ReuseableMethods(bn,v,os);
			}
			obj.launchSite(u);
			obj.fillAndValidate(uid, uidc, pwd, pwdc);
			obj.closeSite();
		}
		wb.close();
		fi.close();
